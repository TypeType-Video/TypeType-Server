...
inline fun <T> withTargetedRequestShape(
...
@PublishedApi internal fun SabrSessionHolder.companionFormat(format: YoutubeSabrFormat): YoutubeSabrFormat =
    if (format.isAudio) videoFormat else audioFormat

@PublishedApi internal fun SabrSegmentRequest.trackMode(): Int =
    if (format.isAudio) YoutubeSabrStreamState.TRACK_MODE_AUDIO_ONLY else YoutubeSabrStreamState.TRACK_MODE_VIDEO_ONLY

@PublishedApi internal fun SabrSegmentRequest.targetRange(holder: SabrSessionHolder): SabrBufferedRange =
    format.bufferedRange(holder, (sequenceNumber - 1).coerceAtLeast(0))

@PublishedApi internal fun YoutubeSabrFormat.targetCompanionRange(holder: SabrSessionHolder, targetPlayerTimeMs: Long): SabrBufferedRange =
    bufferedRange(holder, (holder.playbackStartSequence(this, targetPlayerTimeMs) - 1).coerceAtLeast(0))

@PublishedApi internal fun SabrSegmentRequest.targetPlayerTimeMs(holder: SabrSessionHolder, startMs: Long): Long {
...
@PublishedApi internal fun YoutubeSabrFormat.bufferedRange(holder: SabrSessionHolder, bufferedSequence: Int): SabrBufferedRange {
...
@PublishedApi internal const val SEEK_FORMAT_ORDER_MS = 1_000L
private const val TARGET_SEGMENT_OFFSET_MS = 1_000L
private const val TIMESCALE = 1_000

@PublishedApi internal val logger = LoggerFactory.getLogger("SabrTargetRequestShape")
