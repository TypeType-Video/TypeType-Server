...
data class SabrLivePlaybackSnapshot(
    val active: Boolean,
...
fun SabrSessionHolder.livePlaybackSnapshot(): SabrLivePlaybackSnapshot? {
...
fun SabrSessionHolder.resolvePlaybackStartMs(requestedStartMs: Long): Long {
...
fun SabrSessionHolder.isFutureLiveRequest(request: SabrSegmentRequest): Boolean {
...
fun SabrSessionHolder.isHistoricalLiveRequest(request: SabrSegmentRequest): Boolean {
    if (request.isInitializationSegment) return false
    val live = livePlaybackSnapshot()?.takeIf { it.active } ?: return false
...
fun SabrSessionHolder.liveRetryAfterMs(blockedRequests: List<SabrSegmentRequest> = emptyList()): Long =
