...
fun SabrSessionHolder.markServed(segment: SabrMediaSegment): Unit {
    markServed(segment, activeGeneration())
}

fun SabrSessionHolder.markServed(segment: SabrMediaSegment, generation: Long): Unit {
...
fun SabrSessionHolder.markPrepared(segment: SabrMediaSegment): Unit {
    if (!segment.header.isInitSegment) {
        observeMediaSegment(segment)
        val format = if (audioFormat.itag == segment.header.itag) audioFormat else videoFormat
...
fun SabrSessionHolder.markServed(segment: CachedSabrSegment): Unit {
    markServed(segment, activeGeneration())
}

fun SabrSessionHolder.markServed(segment: CachedSabrSegment, generation: Long): Unit {
    if (!segment.init) {
...
fun SabrSessionHolder.shouldSend(segment: SabrMediaSegment): Boolean {
...
fun SabrSessionHolder.shouldSend(segment: CachedSabrSegment): Boolean {
...
fun bothFormatsKnown(holder: SabrSessionHolder): Boolean {
